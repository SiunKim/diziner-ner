...
class ExperimentDataLoader:
    ...
    def load_all_iteration_data(self) -> Dict[int, Dict[str, Any]]:
        """Load data from all available iterations"""
        iteration_data = {}
    ...
    def setup_equal_axes(ax, all_x_vals: List[float], all_y_vals: List[float], 
        ...
        if equal_axes:
            ...
            ax.plot([min_val, max_val], [min_val, max_val], 'k--', alpha=0.3,
                    label=f'Perfect {metric_name}â€“Gold Agreement')
        else:
...
class VisualizationRenderer:
    ...
    def plot_performance_over_iterations(self, figsize: Optional[Tuple[int, int]] = None):
        """Plot performance evolution over iterations against Gold Standard"""
        ...
        # Plot MV and summary performance
        ...
        # Save plots
        # plt.savefig(f'performance_evolution_{self.config.benchmark}.png', dpi=self.plot_config.dpi)
        # plt.savefig(f'performance_evolution_{self.config.benchmark}.tiff', dpi=self.plot_config.dpi)
        # plt.savefig(f'performance_evolution_{self.config.benchmark}.pdf')
    ...
    def plot_metric_evolution(self, metric_key: str, metric_display_name: str,
        ...
        """Generic method to plot metric evolution (strict span F1, Cohen's kappa, etc.)"""
        if not self.performance_data['iterations']:
            print("No iteration data available for plotting")
            return
        ...
        for i, model_name in enumerate(self.performance_data['models']):
            ...
            ax.annotate(f'{disp}\nIter {iterations_model[0]}', (x_vals[0], y_vals[0]), 
                    xytext=(5, 5), textcoords='offset points', fontsize=8, alpha=0.7)
            if len(iterations_model) > 1:
                ax.annotate(f'Iter {iterations_model[-1]}', (x_vals[-1], y_vals[-1]), 
                        xytext=(5, 5), textcoords='offset points', fontsize=8, alpha=0.7)
        
        # Plot average and MV trajectories
        ...
        # plt.savefig(f'{safe_metric_name}_evolution_{self.config.benchmark}.png', dpi=self.plot_config.dpi)
        # plt.savefig(f'{safe_metric_name}_evolution_{self.config.benchmark}.tiff', dpi=self.plot_config.dpi)
        # plt.savefig(f'{safe_metric_name}_evolution_{self.config.benchmark}.pdf')
...
class IterativeExperimentVisualizer:
    ...
    def plot_pairwise_agreements_over_iterations(self, figsize: Optional[Tuple[int, int]] = None):
        ...
        # Save plots
        # plt.savefig(f'pairwise_agreements_{self.config.benchmark}.png', dpi=self.plot_config.dpi)
        # plt.savefig(f'pairwise_agreements_{self.config.benchmark}.tiff', dpi=self.plot_config.dpi)
        # plt.savefig(f'pairwise_agreements_{self.config.benchmark}.pdf')
    
    def plot_summary_statistics(self, figsize: Optional[Tuple[int, int]] = None):
        """Summary statistics and trends with dual y-axis"""
        ...
        # Unified legend
        lines1 = line_best + line_mv + line_strict + line_kappa + line_var_strict + line_var_kappa
        labels1 = [l.get_label() for l in lines1]
        ax1.legend(lines1, labels1, loc='upper left')

        # Plot 2: Performance improvement heatmap
        if len(iterations) > 1:
            model_improvements = defaultdict(list)
            
            # Collect all possible iteration transitions
            all_transitions = []
            for i in range(len(iterations) - 1):
                all_transitions.append((iterations[i], iterations[i + 1]))
            
            # For each model, calculate improvements for each transition
            for model_name in self.performance_data['models']:
                model_data = sorted(self.performance_data['individual_performance'][model_name],
                                    key=lambda x: x['iteration'])
                
                # Create improvement list for all transitions, using NaN for missing data
                model_improvement_row = []
                for start_iter, end_iter in all_transitions:
                    start_data = next((d for d in model_data if d['iteration'] == start_iter), None)
                    end_data = next((d for d in model_data if d['iteration'] == end_iter), None)
                    
                    if start_data and end_data:
                        improvement = end_data['gold_f1'] - start_data['gold_f1']
                        model_improvement_row.append(improvement)
                    else:
                        model_improvement_row.append(np.nan)
                
                if model_improvement_row:
                    model_improvements[model_name] = model_improvement_row
            ...
            if model_improvements:
                ...
                for model_name, improvements in model_improvements.items():
                    if len(improvements) == len(all_transitions):
                        improvement_matrix.append(improvements)
                        model_names.append(model_name[:10])

                if improvement_matrix and len(improvement_matrix) > 0:
                    improvement_array = np.array(improvement_matrix)
                    masked_array = np.ma.masked_invalid(improvement_array)
                    
                    cmap = plt.cm.get_cmap('RdYlGn').copy()
                    cmap.set_bad(color='lightgray', alpha=0.5)
                    
                    im = ax2.imshow(masked_array, cmap=cmap, aspect='equal')
                    
                    ax2.set_title(f'Gold Standard F1 Improvement Heatmap - {self.config.benchmark.upper()}', fontweight='bold')
                    ax2.set_xlabel('Iteration Transition')
                    ax2.set_ylabel('Models')
                    ax2.set_yticks(range(len(model_names)))
                    ax2.set_yticklabels(model_names)

                    transition_labels = [f"{start}â†’{end}" for start, end in all_transitions]
                    ax2.set_xticks(range(len(transition_labels)))
                    ax2.set_xticklabels(transition_labels)

                    cbar = plt.colorbar(im, ax=ax2)
                    cbar.set_label('Gold Standard F1 Improvement', fontsize=10)
                    
                    ax2.text(0.02, 0.98, 'Gray areas: Missing data', 
                    ...
                    ax2.set_title('Gold Standard F1 Improvement Heatmap', fontweight='bold')

        plt.tight_layout()
        plt.show()
        
        # Save plots
        # plt.savefig(f'summary_statistics_{self.config.benchmark}.png', dpi=self.plot_config.dpi)
        # plt.savefig(f'summary_statistics_{self.config.benchmark}.tiff', dpi=self.plot_config.dpi)
        # plt.savefig(f'summary_statistics_{self.config.benchmark}.pdf')
    ...
    def print_performance_summary(self):
        ...
        print("-" * 80)
        print(f"{'Iteration':<10} {'Best Model':<15} {'Best F1':<10} {'Avg F1':<10} {'MV F1':<10} {'Models':<8}")
        print("-" * 80)
        ...
        for summary in self.performance_data['iteration_summaries']:
            ...
            print(f"{iteration:<10} {best_name:<15} {best_f1:<10.3f} {avg_f1:<10.3f} "
                f"{mv_f1:<10.3f} {model_count:<8}")
        
        # Best Model vs MajorityVote comparison
        print(f"\n{'BEST MODEL vs MAJORITY VOTE COMPARISON':<50}")
        print("-" * 85)
        print(f"{'Iteration':<10} {'Best F1':<10} {'MV F1':<10} {'Difference':<12} {'Winner':<15} {'Advantage':<12}")
        print("-" * 85)
        ...
        for summary in self.performance_data['iteration_summaries']:
            ...
            if mv_f1 > 0:
                difference = best_f1 - mv_f1
                if abs(difference) < 0.001:
                    winner = "Tie"
                    advantage = "Â±0.000"
                elif difference > 0:
                    winner = "Best Model"
                    advantage = f"+{difference:.3f}"
                else:
                    winner = "MajorityVote"
                    advantage = f"{difference:.3f}"
                
                print(f"{iteration:<10} {best_f1:<10.3f} {mv_f1:<10.3f} {difference:+<12.3f} "
                    f"{winner:<15} {advantage:<12}")
            else:
                print(f"{iteration:<10} {best_f1:<10.3f} {'N/A':<10} {'N/A':<12} "
                    f"{'Best Model':<15} {'N/A':<12}")
        
        # Summary statistics for Best vs MV
        valid_comparisons = []
        for summary in self.performance_data['iteration_summaries']:
            best_f1 = summary['best_model'][1]['gold_f1']
            mv_f1 = summary['mv_performance']
            if mv_f1 > 0:
                valid_comparisons.append((best_f1, mv_f1, best_f1 - mv_f1))
        
        if valid_comparisons:
            best_wins = sum(1 for _, _, diff in valid_comparisons if diff > 0.001)
            mv_wins = sum(1 for _, _, diff in valid_comparisons if diff < -0.001)
            ties = len(valid_comparisons) - best_wins - mv_wins
            avg_difference = np.mean([diff for _, _, diff in valid_comparisons])
            
            print("-" * 85)
            print(f"{'SUMMARY:':<10} Best Model wins: {best_wins}, MajorityVote wins: {mv_wins}, "
                f"Ties: {ties}, Avg difference: {avg_difference:+.3f}")
        
        # Model improvement analysis
        print(f"\n{'MODEL IMPROVEMENT ANALYSIS (vs Gold Standard)':<50}")
        print("-" * 60)
        print(f"{'Model':<20} {'First F1':<10} {'Last F1':<10} {'Total Î”':<10} {'Avg Î”':<10}")
        print("-" * 60)
        
        for model_name in self.performance_data['models']:
            model_data = self.performance_data['individual_performance'][model_name]
            if len(model_data) >= 2:
                model_data_sorted = sorted(model_data, key=lambda x: x['iteration'])
                first_f1 = model_data_sorted[0]['gold_f1']
                last_f1 = model_data_sorted[-1]['gold_f1']
                total_improvement = last_f1 - first_f1
                avg_improvement = total_improvement / (len(model_data_sorted) - 1)
                
                print(f"{model_name[:19]:<20} {first_f1:<10.3f} {last_f1:<10.3f} "
                    f"{total_improvement:+<10.3f} {avg_improvement:+<10.3f}")
...
if __name__ == "__main__":
    ...
    for benchmark in [
        ...
        visualizer = create_visualization(
            ...
            # supervisor_model_name='gpt-5-nano-2025-08-07',
        ...
        #     starting_group_index=0,
        #     max_iterations=3,
        #     max_common_instructions=8,
        #     max_patterns=10,
        #     model_specific_for_all=False,
        #     max_model_specific_instructions=5,
        #     limit_instruction_changes=True,
        #     max_change_ratio=0.1,
        #     drop_worst_annr=False,
        #     supervised_by_gold_standard=False,
        #     supervisor_model_name='gpt-4o-mini-2024-07-18',
        #     # supervisor_model_name='gpt-5-2025-08-07',
        #     plot_config=custom_plot_config
        # )